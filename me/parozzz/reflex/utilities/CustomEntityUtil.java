...
public class CustomEntityUtil {
	...
	public static void registerEntity(Type type, Class<? extends Entity> customClass, boolean override) {
		CustomEntityUtil.registerEntity(type.getName(), type, customClass, override);
	}
	...
	public static void registerEntity(String name, Type type, Class<? extends Entity> customClass, boolean override) {
		...
		try {
			field.setAccessible(true);
			for (BiomeBase base : CustomEntityUtil.BIOMES) {
				List<BiomeMeta> list = (List<BiomeMeta>) field.get(base);
				for (BiomeMeta meta : list) {
					if (meta.b == type.getNMSClass()) {
						meta.b = (Class<? extends EntityInsentient>) customClass;
	...
	public enum MobMeta {
		MONSTER(CustomEntityUtil.META_LIST_MONSTER),
		CREATURE(CustomEntityUtil.META_LIST_CREATURE),
		WATER_CREATURE(CustomEntityUtil.META_LIST_WATER_CREATURE),
		AMBIENT(CustomEntityUtil.META_LIST_AMBIENT),
		UNDEFINED(null);
	...
	static {
		...
		MINECRAFT_SERVER = CustomEntityUtil.CRAFTBUKKIT_SERVER.getServer();
		BIOMES = new BiomeBase[BiomeBase.i.a()];
		Iterator<BiomeBase> iterator = BiomeBase.i.iterator();
		int index = 0;
		while (iterator.hasNext()) {
			CustomEntityUtil.BIOMES[index++] = iterator.next();
		}
	}
}